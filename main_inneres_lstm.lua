...
cmd:option('-feature_maps', '{50, 100, 100, 100, 100, 100, 100}', 'for keep the same with 650')
...
inneresLSTMTDNN = require 'model.inneresLSTMTDNN'
--HighwayMLP = require 'model.HighwayMLP'
ResMLP  = require 'model.ResMLP'
...
protos.rnn = inneresLSTMTDNN.innereslstmtdnn(opt.rnn_size, opt.num_layers, opt.dropout, #loader.idx2word, 
...
for i = 1, iterations do
    ...
    if i % loader.split_sizes[1] == 0 and #val_losses > 2 then
        if val_losses[#val_losses-1] - val_losses[#val_losses] < opt.decay_when then
            ...
            print('learning_rate now = ', lr)
        end
        if lr < 0.001 then print(lr, 'need to stop') break end
    end    

    if i % opt.print_every == 0 then
        avg_train_loss = sum_train_loss * 1.0 / #train_losses
        print(string.format("%d/%d (epoch %.2f), train_loss = %6.4f", i, iterations, epoch, avg_train_loss))
    end   
